...
# Verification is deliberately one-click OAuth only. The old CAPTCHA modes
# remain readable during migration but are normalized to OAuth before use.
METHODS = ("oauth",)
...
DEFAULTS: dict[str, Any] = {
    ...
    # Per-server OAuth membership blacklist. Only guild ids selected by this
    # server's administrators are checked; Discord passwords, email addresses,
    # messages, and unrelated profile data are never requested.
    "server_blacklist_enabled": False,
...
TEXT_KEYS = (
    ...
    "blacklist_title", "blacklist_text",
)
...
PLACEHOLDERS = {
    ...
    "{blocked_server}": "Server, der auf der Blacklist gefunden wurde",
}
...
async def ensure_schema(db: aiosqlite.Connection) -> None:
    ...
    wanted = {
        ...
        "server_blacklist_enabled": "INTEGER DEFAULT 0",
        "blacklisted_guild_ids": "TEXT DEFAULT '[]'",
...
def normalise(settings: dict) -> dict:
    ...
    if isinstance(raw_blocked, str):
        try:
            ...
            raw_blocked = []
    out["blacklisted_guild_ids"] = list(dict.fromkeys(
        str(item) for item in (raw_blocked if isinstance(raw_blocked, list) else [])
        if str(item).isdigit()
    ))[:250]
...
def render(text: str, *, server: str = "", user_mention: str = "",
           user_name: str = "", role: str = "", member_count: int = 0,
           blocked_server: str = "") -> str:
    """
...
def methods_for(settings: dict) -> list[str]:
    """The sole supported verification method."""
    return ["oauth"]
...
def readiness(settings: dict) -> list[str]:
    ...
    if settings.get("server_blacklist_enabled"):
        if not settings.get("blacklisted_guild_ids"):
            problems.append("Server-Blacklist ist an, enthält aber noch keinen Server.")
        if not settings.get("blacklist_log_channel_id") \
                and not settings.get("log_channel_id"):
            problems.append("Server-Blacklist ist an, aber es ist kein Log-Kanal gesetzt.")
