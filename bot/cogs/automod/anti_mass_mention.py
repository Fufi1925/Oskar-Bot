...
class AntiMassMention(commands.Cog):
    """
    Pinging a lot of people at once.
    ...
    async def on_message(self, message):
        ...
        # Counts people and roles, not the raw text: five copies of the
        # same mention is one person being pinged five times.
        mentioned = {u.id for u in message.mentions}
        mentioned |= {r.id for r in message.role_mentions}
        if message.mention_everyone:
            mentioned.add(0)
        if len(mentioned) < entry["threshold"]:
            return

        action = await store.punish(
            self.bot, message, self.RULE, settings, "Massen-Erwähnung"
        )
        if action is None:
            return

        await store.log_action(
            self.bot, message, settings, self.RULE, action, "Massen-Erwähnung"
        )
        ...
        try:
            ...
            await message.channel.send(
                view=Panel(
                    "Automod",
                    f"{member.mention} — {action} ({'Massen-Erwähnung'}).",
                    tone="warning",
                ),
                delete_after=15,
            )
