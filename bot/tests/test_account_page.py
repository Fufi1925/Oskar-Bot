...
check("five useful actions are rendered", actions.count("label:") == 4 and "Abmelden" in danger)
check("the danger zone has two confirmations and exact username", "Bist du sicher?" in danger and "Unwiderruflich bestätigen" in danger and "typed !== username" in danger)
check("the deletion request has a ten-second undo", "Rückgängig machen" in danger and "10-Sekunden-Rücknahmefrist" in danger)
check("retained data is explained", all(word in danger for word in ("XP", "Voice-Zeit", "Servereinstellungen", "Art. 17 Abs. 3")))
