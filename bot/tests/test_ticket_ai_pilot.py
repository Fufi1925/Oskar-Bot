...
check("missing prerequisites explain instead of silently disabling", "Lade zuerst eine .txt-Wissensdatei hoch" in PANEL and "XAI_TICKET_AI_KEY fehlt oder enthält keinen gültigen xAI-Key" in PANEL)
...
check("Railway secret uses a valid environment name", 'API_KEY_ENV = "XAI_TICKET_AI_KEY"' in AI)
check("Grok uses current models with a fallback", 'DEFAULT_MODEL = "grok-4.6"' in AI and '"grok-4.3"' in AI and "model_candidates()" in AI and "response.status_code == 404" in AI)
check("xAI key never appears in request URLs or errors", '"Authorization": f"Bearer {key}"' in AI and 'params={"key": key}' not in AI and "XAI_CHAT_URL" in AI)
check("Google keys are rejected before a scan", 'key.startswith("xai-")' in AI and "keinen gültigen xAI-Key" in API)
check("HTTP 400 retries without optional model parameters", "response.status_code == 400" in AI and 'json={"model": model, "messages": messages, "stream": False}' in AI and "_xai_error(response)" in AI)
check("Ticket AI contains no Google or Gemini integration", all(word not in (AI + SCAN + API + PANEL + ADMIN_PANEL).lower() for word in ("google", "gemini", "generativelanguage")))
...
check("claim race is rechecked after Grok", "Claim can happen while Grok is working" in TICKET)
check("categories independently enable AI", "ticket_ai_categories" in AI and 'config["category_enabled"]' in TICKET)
check("Grok receives excerpts instead of complete knowledge", "matching_context" in AI and "excerpts" in TICKET and "source[:5000]" in AI)
