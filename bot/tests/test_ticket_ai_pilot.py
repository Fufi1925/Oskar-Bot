...
check("retired Gemini IDs fall back to stable models", 'DEFAULT_MODEL = "gemini-2.5-flash"' in AI and "model_candidates()" in AI and "candidate.status_code == 404" in SCAN)
check("Google key never appears in request URLs or errors", 'headers={"x-goog-api-key": key}' in AI and 'params={"key": key}' not in AI and 'params={"key": key}' not in SCAN)
