...
check("scan requests stay below the Groq on-demand TPM ceiling", "CHUNK_SIZE = 4_000" in SCAN and "max_tokens=1200" in SCAN and "for offset in range" in SCAN and "for offset in range(0, max(1, len(part)), CHUNK_SIZE)" in SCAN)
check("scan forces a non-empty structured Groq result", "json_mode=True" in SCAN and 'parsed.get("text")' in SCAN and "gültiges JSON-Objekt" in SCAN)
check("scan reduction terminates and exposes AI progress", "else 1900" in SCAN and "remaining = (remaining + 1) // 2" in SCAN and "total_channels=total_calls" in SCAN and "_reduce(parts, guild_id)" in SCAN and "KI-Schritten abgeschlossen" in PANEL)
