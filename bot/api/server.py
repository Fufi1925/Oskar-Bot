...
def create_app() -> FastAPI:
    ...
    api_app.include_router(vanity.router, prefix="/vanity", tags=["Vanity Roles"])
    api_app.include_router(
    ...
    api_app.include_router(voice.router, prefix="/voice", tags=["Voice"])
    api_app.include_router(
        speedrun.router, prefix="/speedrun", tags=["Speedrun"]
    )
    api_app.include_router(verify.router, prefix="/verify", tags=["Verification"])
    api_app.include_router(automod.router, prefix="/automod", tags=["Automod"])
    api_app.include_router(
        logging_cfg.router, prefix="/logging", tags=["Logging"]
    )
    api_app.include_router(
        antinuke.router, prefix="/antinuke", tags=["Anti-Nuke"]
    )
    # Broadcasts sit under /admin so the owner-only gate in the dashboard
    # proxy already covers them.
    api_app.include_router(
        broadcast.router, prefix="/admin", tags=["Admin"]
    )
    api_app.include_router(
    ...
    # Das Design des Bots je Server. Eigenes Praefix mit guild_id im
    ...
    # Sprachkanäle mit Menschen-, Bot- und Gesamtzahl je Server.
    api_app.include_router(
        server_stats.router, prefix="/server-stats", tags=["Server Stats"]
    )
    # Die Beta-Antraege fuer Hauptbot-Premium. Bewusst NICHT unter
    # /admin: die Antragsseite steht jedem Angemeldeten offen, nur
    # /beta/admin/* verlangt Admin-Rechte -- das prueft der Proxy.
    api_app.include_router(
        beta.router, prefix="/beta", tags=["Beta"]
    )
    ...
    api_app.include_router(
        teamlist.router, prefix="/teamlist", tags=["Teamlist"]
    )
    ...
    async def proxy_to_dashboard(request: Request, path: str):
        ...
        try:
            ...
            return Response(
                content="""<!DOCTYPE html><html><head><title>University Bot</title>\n<link rel="icon" type="image/png" sizes="32x32" href="data:image/png;base64,">\n<style>body{font-family:system-ui;display:flex;justify-content:center;align-items:center;height:100vh;background:#0a0a0a;color:#fff;margin:0}
