...
export const api = {
  ...
  runFirewallSelfProtectionTest: () => request<any>("/firewall/self-protection-test"),
  getFirewallAudit: (limit = 300) => request<any>(`/firewall/audit?limit=${limit}`),
  rollbackFirewallAudit: (id: number, reason: string) => request<any>(`/firewall/audit/${id}/rollback`, { method: "POST", body: JSON.stringify({ reason }) }),
  compareFirewallSnapshot: (id: number) => request<any>(`/firewall/snapshots/${id}/compare`),
  acknowledgeFirewallIncident: (id: number) => request<any>(`/firewall/incidents/${id}/acknowledge`, { method: "POST", body: "{}" }),
  trustFirewallIncident: (id: number, scope: "ip" | "user") => request<any>(`/firewall/incidents/${id}/trust`, { method: "POST", body: JSON.stringify({ scope }) }),
  editFirewallRule: (id: number, data: any) => request<any>(`/firewall/rules/${id}`, { method: "PATCH", body: JSON.stringify(data) }),
  cloneFirewallRule: (id: number, data: any) => request<any>(`/firewall/rules/${id}/clone`, { method: "POST", body: JSON.stringify(data) }),
  annotateFirewallEvent: (id: number, note: string) => request<any>(`/firewall/events/${id}/note`, { method: "PATCH", body: JSON.stringify({ note }) }),
  setTimedFirewallEmergency: (minutes: number) => request<any>("/firewall/operations/timed-emergency", { method: "POST", body: JSON.stringify({ minutes }) }),
  getFirewallSafetyAudit: () => request<any>("/firewall/safety-audit"),
  resetFirewallCounters: () => request<any>("/firewall/operations/reset-counters", { method: "POST", body: "{}" }),
  ...
  uploadTicketAiKnowledge: (guildId: string, filename: string, content: string) =>
    request<any>(`/tickets/${guildId}/ai/knowledge`, {
      method: "PUT",
