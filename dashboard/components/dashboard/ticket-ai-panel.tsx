...
export function TicketAiPanel({ guildId }: { guildId: string }) {
  ...
  const startScan = async () => {
    if (!data?.api_key_configured) {
      toast.error("Die Railway-Variable XAI_TICKET_AI_KEY fehlt oder enthält keinen gültigen xAI-Key.");
      return;
    }
      ...
      {loading ? (
        ...
        <div className="space-y-6 p-5 sm:p-6">
          {!data.api_key_configured && (
            <div className="rounded-2xl border border-amber-400/25 bg-amber-400/[0.06] p-4 text-sm text-amber-200">Railway-Variable <b>XAI_TICKET_AI_KEY</b> fehlt oder enthält keinen gültigen xAI-Key. Nach dem Eintragen den Bot-Dienst neu starten.</div>
          )}
