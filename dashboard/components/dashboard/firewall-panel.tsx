...
import {
  ...
  Search, Shield, ShieldCheck, Trash2, Undo2, UserRoundX, Wifi, X, Zap, Settings2, Download, Eraser, EyeOff,
} from "lucide-react";
import { toast } from "sonner";
import { api } from "@/lib/api";
import { cn } from "@/lib/utils";

type Tab = "overview" | "bans" | "trusted" | "protection" | "events" | "operations" | "diagnostics";
...
export function FirewallPanel() {
  ...
  const [ruleEdit,setRuleEdit] = useState({minutes:0,note:""});
  ...
  return <div className={cn("space-y-5",busy && "pointer-events-none opacity-75")}>
    ...
    <nav className="flex gap-1 overflow-x-auto rounded-2xl border border-slate-800 bg-[#131318] p-1.5">
      {([
        ...
        ["events","Ereignisse",Clock3],["operations","Werkzeuge",Settings2],["diagnostics","API-Diagnose",Bot],
    ...
    {selectedRule && <Modal close={()=>setSelectedRule(null)}><Settings2 className="h-8 w-8 text-cyan-300"/><h3 className="mt-4 text-xl font-black text-white">Regel #{selectedRule.id} bearbeiten</h3><p className="mt-1 font-mono text-sm text-slate-400">{selectedRule.value}</p><Field label="Neue Dauer in Minuten · 0 = dauerhaft"><input className="input" type="number" min="0" value={ruleEdit.minutes} onChange={event=>setRuleEdit({...ruleEdit,minutes:Number(event.target.value)})}/></Field><Field label="Interne Notiz"><textarea className="input min-h-24" value={ruleEdit.note} onChange={event=>setRuleEdit({...ruleEdit,note:event.target.value})}/></Field><div className="mt-6 flex justify-end gap-2"><button onClick={()=>setSelectedRule(null)} className="secondary">Abbrechen</button><button onClick={saveRuleEdit} className="primary"><Save className="h-4 w-4"/>Änderung speichern</button></div></Modal>}
