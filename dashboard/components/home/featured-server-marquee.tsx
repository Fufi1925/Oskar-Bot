...
export function FeaturedServerMarquee() {
  ...
  return <section className="overflow-hidden border-b border-blue-500/10 bg-[#0b0a0c] py-10 sm:py-20">
    <div className="mx-auto mb-5 max-w-[1320px] px-4 text-center sm:mb-9 sm:px-6"><p className="text-[10px] font-black uppercase tracking-[.28em] text-blue-400">Community</p><h2 className="mt-2 text-2xl font-black tracking-tight text-white sm:mt-3 sm:text-4xl">Server, die uns vertrauen</h2></div>
    <div className="server-marquee flex w-max px-2">
      {[0, 1, 2].map((copy) => <div key={copy} className="flex shrink-0 gap-4 pr-4" aria-hidden={copy > 0}>{servers.map((server) => <article key={`${copy}-${server.guild_id}`} className="flex w-[240px] shrink-0 items-center gap-3 rounded-2xl border border-white/[.08] bg-[#121116] p-3 shadow-[0_12px_35px_rgba(0,0,0,.22)] sm:w-[320px] sm:p-4">
    ...
    <style jsx global>{`@keyframes featuredServerRun { from { transform: translate3d(0,0,0); } to { transform: translate3d(-33.333333%,0,0); } } .server-marquee { animation: featuredServerRun ${Math.max(26, servers.length * 5)}s linear infinite !important; animation-play-state: running !important; will-change: transform; backface-visibility: hidden; }`}</style>
  </section>;
}
