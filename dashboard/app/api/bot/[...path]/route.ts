...
async function authorize(
  ...
  if (scope === "guild-access") {
    ...
    // Discord's actual server owner can view and change the grants.
    const guildId = rest[0] ?? "";
    const session = await getServerSession(authOptions);
    if (!session?.user?.id) return { ok: false, response: deny(401, "Not signed in.") };
  ...
  if (scope === "server-stats") {
    // Shape: /server-stats/<guildId>. Lesen braucht Zugriff auf den Server,
    // Änderungen zusätzlich die normale Berechtigung für Einstellungen.
    const guildId = rest[0];
    if (!guildId) return { ok: false, response: deny(400, "guild_id missing.") };
  ...
  if (scope === "backup") {
    // Shape: /backup/<guildId>/...
    //
    // Eine Sicherung liest den ganzen Server aus, und das
    // Wiederherstellen kann ihn leerräumen. Das ist die
    // eingreifendste Aktion nach dem Speedrun -- entsprechend hoch
    // liegt die Schwelle: wer den Server auf Discord verwaltet, oder
    // eine Team-Rolle mit `server.manage`.
    //
    // Lesen (GET) reicht `guild.view`: die Liste zeigt nur Kennungen
    // und Zeitpunkte, keinen Inhalt.
    const guildId = rest[0];
    if (!guildId) return { ok: false, response: deny(400, "guild_id missing.") };
    ...
    const team = await fetchTeamAccess(session.user.id);
    if (!team || team.roles.length === 0) {
      return { ok: false, response: deny(403, "This requires managing the server.") };
    }
