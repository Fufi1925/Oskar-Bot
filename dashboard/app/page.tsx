      ...
      <section className="relative flex min-h-[790px] items-center overflow-hidden px-4 pb-16 pt-28 sm:px-6 lg:pt-32">
        ...
        <div className="relative mx-auto grid w-full max-w-[1320px] items-center gap-14 lg:grid-cols-[1.05fr_.95fr]">
          ...
          <div className="relative min-h-[470px] sm:min-h-[540px]">
            <div className="absolute left-1/2 top-1/2 h-[300px] w-[300px] sm:h-[390px] sm:w-[390px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-fuchsia-500/15 bg-fuchsia-500/[.025] shadow-[0_0_120px_rgba(192,38,211,.16)]"><div className="absolute inset-10 rounded-full border border-dashed border-fuchsia-400/20" /><div className="absolute inset-24 grid place-items-center rounded-[52px] border border-fuchsia-400/25 bg-[#171019] shadow-[0_0_70px_rgba(217,70,239,.18)]"><img src="/icon-192.png" alt="" className="h-28 w-28 rounded-3xl object-cover" /></div></div>
            {[
            ...
            ].map(([label, Icon, pos]) => { const FeatureIcon = Icon as React.ElementType; return <div key={String(label)} className={`absolute ${pos} flex min-w-36 items-center gap-2 sm:min-w-48 sm:gap-3 rounded-2xl border border-fuchsia-400/20 bg-[#171219]/90 p-3 sm:p-4 shadow-[0_18px_55px_rgba(0,0,0,.5)] backdrop-blur-xl`}><span className="grid h-11 w-11 place-items-center rounded-xl border border-fuchsia-400/20 bg-fuchsia-500/10"><FeatureIcon className="h-5 w-5 text-fuchsia-400" /></span><div><p className="text-sm font-black text-white">{String(label)}</p><p className="mt-0.5 text-[10px] uppercase tracking-widest text-zinc-600">Live Modul</p></div></div>; })}
          </div>
        </div>
      </section>
