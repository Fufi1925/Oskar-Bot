...
function NewHomepage() {
  ...
  return (
    <main className="new-home min-h-screen overflow-x-clip bg-[#09090b] text-zinc-200 selection:bg-blue-500/30">
      ...
      <section
        className="new-home-universe relative flex min-h-[790px] items-center overflow-hidden bg-cover px-4 pb-8 pt-24 sm:px-6 sm:pb-16 sm:pt-28 lg:pt-32"
      ...
      <section id="features" className="px-4 py-12 sm:px-6 sm:py-28"><div className="mx-auto max-w-[1200px]"><div className="max-w-2xl"><p className="text-xs font-black uppercase tracking-[.24em] text-blue-400">Alles verbunden</p><h2 className="mt-3 text-3xl font-black tracking-tight text-white sm:mt-4 sm:text-5xl">Ein Bot statt eines ganzen Bot-Ordners.</h2><p className="mt-3 text-sm leading-6 text-zinc-400 sm:mt-5 sm:text-base sm:leading-7">Die wichtigsten Werkzeuge für Aufbau, Schutz und Betrieb deines Discord-Servers greifen ineinander.</p></div><div className="-mx-4 mt-7 flex snap-x snap-mandatory gap-3 overflow-x-auto px-4 pb-3 [scrollbar-width:none] md:mx-0 md:mt-12 md:grid md:overflow-visible md:px-0 md:pb-0 md:grid-cols-2 lg:grid-cols-3">{FEATURES.map((feature,index)=><article key={feature.title} className={`group relative w-[82vw] max-w-[320px] shrink-0 snap-center overflow-hidden rounded-2xl border border-white/[.07] bg-[#100d11] p-5 transition hover:border-blue-400/25 md:w-auto md:max-w-none md:p-6 ${index===0||index===5 ? "lg:col-span-1" : ""}`}><div className="absolute -right-12 -top-12 h-32 w-32 rounded-full bg-blue-500/[.06] blur-2xl transition group-hover:bg-blue-500/10"/><span className="grid h-11 w-11 place-items-center rounded-xl border border-blue-400/20 bg-blue-500/10"><feature.icon className="h-5 w-5 text-blue-400"/></span><h3 className="mt-4 text-lg font-black text-white sm:mt-6">{feature.title}</h3><p className="mt-2 text-sm leading-6 text-zinc-500 sm:mt-3 sm:leading-7">{feature.text}</p><span className="mt-4 sm:mt-6 inline-flex items-center gap-2 text-xs font-black text-blue-400">Im Dashboard verwalten<ArrowRight className="h-3.5 w-3.5"/></span></article>)}</div></div></section>

      <section id="premium" className="px-4 py-12 sm:px-6 sm:py-28"><div className="relative mx-auto max-w-[1200px] overflow-hidden rounded-[32px] border border-amber-400/20 bg-[#14130d] p-5 sm:p-12"><div className="pointer-events-none absolute -right-28 -top-40 h-96 w-96 rounded-full bg-amber-600/15 blur-3xl"/><div className="relative grid gap-7 sm:gap-10 lg:grid-cols-[1fr_.8fr] lg:items-center"><div><p className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[.22em] text-amber-400"><Crown className="h-4 w-4"/>Premium</p><h2 className="mt-4 text-3xl font-black tracking-tight text-white sm:mt-5 sm:text-5xl">Mehr Freiheit für deine wichtigsten Server.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-400 sm:mt-5 sm:text-base sm:leading-8">Drei feste Serverplätze, höhere Limits, individuelles Bot-Design, erweiterte Backups, Statistiken und Premium-Werkzeuge.</p><Link href="/premium" className="mt-5 sm:mt-8 inline-flex items-center gap-2 rounded-xl bg-amber-600 px-6 py-3.5 text-sm font-black text-white hover:bg-amber-500">Premium ansehen<ArrowRight className="h-4 w-4"/></Link></div><div className="grid grid-cols-2 gap-2 sm:gap-3">{["3 feste Serverplätze","Eigenes Bot-Design","Erweiterte Backups","Mehr Custom Commands","Server-Statistiken","User Pull für Owner"].map(item=><div key={item} className="flex items-start gap-2 rounded-xl border border-white/[.07] bg-black/20 p-3 text-xs font-bold leading-5 text-zinc-300 sm:items-center sm:gap-3 sm:p-4 sm:text-sm"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-amber-500/10"><Check className="h-4 w-4 text-amber-400"/></span>{item}</div>)}</div></div></div></section>

      <section id="faq" className="px-4 py-12 sm:px-6 sm:py-28"><div className="mx-auto grid max-w-[1100px] gap-7 sm:gap-12 lg:grid-cols-[.65fr_1fr]"><div><p className="text-xs font-black uppercase tracking-[.24em] text-blue-400">FAQ</p><h2 className="mt-3 text-3xl font-black tracking-tight text-white sm:mt-4 sm:text-4xl">Noch Fragen?</h2><p className="mt-4 text-sm leading-7 text-zinc-500">Die wichtigsten Antworten kurz und ohne Kleingedrucktes.</p><a href={SUPPORT_INVITE} target="_blank" rel="noreferrer" className="mt-4 sm:mt-6 inline-flex items-center gap-2 text-sm font-black text-blue-400"><MessageSquareText className="h-4 w-4"/>Support kontaktieren</a></div><div className="rounded-2xl border border-white/[.07] bg-[#100d11] px-4 sm:px-6">{FAQ.map(([q,a])=><FaqRow key={q} question={q} answer={a}/>)}</div></div></section>

      <section className="px-4 pb-16 sm:px-6 sm:pb-24"><div className="mx-auto flex max-w-[1200px] flex-col items-center rounded-[30px] border border-blue-400/20 bg-[radial-gradient(circle_at_50%_0%,rgba(96,165,250,.15),transparent_60%),#110d12] px-5 py-10 text-center sm:px-6 sm:py-16"><Languages className="h-7 w-7 text-blue-400"/><h2 className="mt-4 text-2xl font-black text-white sm:mt-5 sm:text-4xl">Dein Server kann einfacher laufen.</h2><p className="mt-3 max-w-xl text-sm leading-6 sm:mt-4 sm:leading-7 text-zinc-400">Lade {BRAND} ein und richte genau die Module ein, die du brauchst.</p><a href={INVITE_URL} target="_blank" rel="noreferrer" className="mt-5 inline-flex sm:mt-7 items-center gap-2 rounded-xl bg-blue-600 px-7 py-3.5 text-sm font-black text-white hover:bg-blue-500">Bot jetzt einladen<ArrowRight className="h-4 w-4"/></a></div></section>
