...
Bewusst NICHT aufgenommen:
  - lib/announcements.ts        -> Changelog-Archiv, historischer Inhalt
  - app/imprint, privacy, terms -> Rechtsseiten (Impressum muss deutsch
                                   bleiben; Datenschutz/AGB siehe Filter
                                   weiter unten)
...
def add(text, rel):
    t = norm(text)
    if len(t) < 2 or t in known or is_technical(t):
        return
    if not GERMAN_HINT.search(t):
        return
